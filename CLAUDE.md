...
项目把无头部、X-Y-Z 顺序的原始 `float32` `.dat` 转换成 `.b3d`：固定立方体块数据按 Morton/Z-order 写入，逻辑块索引表保存每个块的绝对文件字节偏移。`.b3d` 支持 v1 legacy XYZ 块内布局和 v2 micro-tiled XYZ 块内布局；当前 CLI 默认转换仍生成 v1，显式 `--layout micro-tiled --micro-size 8` 生成 v2。`BlockedFileReader` mmap 整个 `.b3d`，按索引定位块并根据 header 中的布局信息提供 X/Y/Z 切片、列、子体积、单点和完整体积读取。
...
- `.b3d` 头固定 64 字节，magic 为 `3DBK`，数据区按 4096 字节对齐；v1 为 legacy XYZ，v2 通过 `reserved` 编码 micro-tiled layout 和 micro size。
- 索引表按逻辑块 X-Y-Z 顺序排列；块数据按 Morton 顺序排列；索引值是绝对字节偏移。
- v1 块内为局部 X-Y-Z 且 Z 连续；v2 micro-tiled 块内先按 8³ micro tile 展平再在 micro 内保持 Z 连续；边界块必须补零并在读取时按原始尺寸裁剪。
...
- Phase 5 正式实验证据已归档到 `experiments/批量融合读取与读写流水线优化开发方案实验数据归档/`：只包含文本日志、派生 CSV/TXT 和报告说明，不包含大型 `.raw` 输出。后续写报告或复核 dispatch 默认策略时优先引用该归档。
- Phase 6 micro-tiled v2 A/B 证据已归档到 `experiments/phase6_20260717/`：包含原实现方案、test18/test50 legacy vs micro8 日志副本和派生 CSV。test18/test50 的 `run_test` 中位数显示 micro8 总体通过守门，但 X-heavy 负载仍可能退化。
...
- 修改格式、布局、转换或 reader：运行 `test_block3d` 和 `test_cli`；如果改动会影响 v2 micro-tiled 性能路径，再用 `run_test --b3d-file` 针对 micro8 文件做 smoke 或正式 A/B。
