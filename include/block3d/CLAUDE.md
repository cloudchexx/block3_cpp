...
- 块内布局由 header 版本/layout metadata 决定：v1 为 legacy 局部 X-Y-Z，Z 连续；v2 为 fixed micro-tiled XYZ（当前只允许 `micro_size=8`）。切片和列 API 的返回顺序是公共契约，不随磁盘块内布局变化。
...
- Phase 5 正式 A/B 证据归档在 `../../experiments/批量融合读取与读写流水线优化开发方案实验数据归档/`；公共 API 层保持两种 dispatch 可选，但默认策略由上层 benchmark 证据固定为 round-robin。
- Phase 6 micro-tiled v2 A/B 证据归档在 `../../experiments/phase6_20260717/`；公共 API 层同时支持 v1/v2，转换默认仍为 v1，默认是否切换应由上层策略和 X-heavy 风险决定。
