...
构造：读取头和索引表，再 mmap 整个 `.b3d`，并按 `num_threads_` 创建 reader 生命周期内的持久块处理线程池。
...
- `read_slices_batch()` / `read_slices_batch_stream()`：先预校验 axis 和整批 index，按 `(axis, index / block_size)` 首次出现顺序分组，再按 `window_slices` 融合遍历同一 layer 的物理排序块；兼容 API 按 `request_pos` 收集结果；`options.num_threads` 非零时作为本次调用的 worker 上限。
...
- 默认保存控制台日志到当前工作目录 `logs/`，包括 stdout/stderr；Phase 5 正式日志另有报告归档副本位于 `../experiments/phase5_20260717/`；
