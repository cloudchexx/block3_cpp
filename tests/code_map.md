...
`run_test` 不在本目录，也不是 CTest；它是大数据性能工具。修改 `run_test` 的 cold/hot、输出目录或日志行为后，应单独运行 smoke benchmark；需要性能确认时再使用 `block3d-data/test18` 与 `block3d-cache/scrub.bin` 跑完整流程。Phase 5 正式 benchmark 的报告归档位于 `../experiments/phase5_20260717/`，与本目录小型测试夹具分开维护。